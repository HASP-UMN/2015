...
//  //Serial.println("=====================================================");
  ...
  //Serial.println("Going to sleep for 3 minutes to wait for Tomcat to boot.");
  ...
  bool sec_change = false;
    ...
    //Serial.println("Requesting init_time2 (1st byte from RTC)");
    init_time2 = Wire.read();
    //Serial.print("  init_time2 = "); //Serial.print(init_time2,BIN); //Serial.print("("); //Serial.print(init_time2,DEC); //Serial.println(")");
  // while loop must poll RTC to get most accurate second change nearest to first gpsPPS tic


  //Serial.println("Entering while(sec_change == false) loop");
  while(sec_change == false){ 
    //Serial.print("  sec_change = "); //Serial.print(sec_change,BIN); //Serial.print("("); //Serial.print(sec_change,DEC); //Serial.println(")");

    
    init_time1 = init_time2;
    start_usec_offset = micros();
    //Serial.print("  init_time1 = "); //Serial.print(init_time1,BIN); //Serial.print("("); //Serial.print(init_time1,DEC); //Serial.println(")");
    //Serial.print("  start_usec_offset = "); //Serial.print(start_usec_offset,BIN); //Serial.print("("); //Serial.print(start_usec_offset,DEC); //Serial.println(")");

    //Serial.println("  Before reading from RTC:");
    Wire.beginTransmission(DS3231);
    Wire.write(0);
    Wire.endTransmission();
    Wire.requestFrom(DS3231, 1);
    init_time2 = Wire.read(); // polls from RTC "seconds" register
    //Serial.println("  After reading from RTC:");
    //Serial.print("  init_time2 = "); //Serial.print(init_time2,BIN); //Serial.print("("); //Serial.print(init_time2,DEC); //Serial.println(")");
    ...
    //Serial.println("Exiting while(sec_change == false) loop");
  while(ticCount == 0){
    /*
     ...
     */
     ////Serial.println("ticCount = 0");
    }
    //Serial.print("ticCount = "); //Serial.print(ticCount,BIN); //Serial.print("("); //Serial.print(ticCount,DEC); //Serial.println(")");
    ...
    //Serial.print("Hours = "); //Serial.print(Hours,BIN); //Serial.print("("); //Serial.print(Hours,DEC); //Serial.println(")");
    //Serial.print("Minutes = "); //Serial.print(Minutes,BIN); //Serial.print("("); //Serial.print(Minutes,DEC); //Serial.println(")");
    //Serial.print("Seconds = "); //Serial.print(init_time2,BIN); //Serial.print("("); //Serial.print(init_time2,DEC); //Serial.println(")");
    //start_time =  Minutes<<8 | init_time2;
    
    // Sends initialization and time zero packet to the TOMCAT;    
    send_start_time(Hours, Minutes, init_time2, start_usec_offset);
...
void send_data(ADC_data* data, unsigned int checksum){
    ...
    PORTF = data->send_channel;               //1st byte        
    PORTH = FIFO_WR_ASSERT; 
    PORTH = FIFO_WR_DEASSERT; 

    PORTF = (ticStamp  & 0xFF00)>>8;          //2nd byte        
    PORTH = FIFO_WR_ASSERT; 
    PORTH = FIFO_WR_DEASSERT;
    ...
    PORTF = (uSecStamp  & 0xFF0000)>>16;      //4th byte    
    PORTH = FIFO_WR_ASSERT; 
    PORTH = FIFO_WR_DEASSERT;
    ...
    PORTF = (uSecStamp  & 0xFF);              //6th byte        
    PORTH = FIFO_WR_ASSERT; 
    PORTH = FIFO_WR_DEASSERT; 

    PORTF = (data->peak_val & 0xFF00)>>8;     //7th byte        
    PORTH = FIFO_WR_ASSERT; 
    PORTH = FIFO_WR_DEASSERT;
    
    PORTF = (data->peak_val & 0xFF);          //8th byte    
    PORTH = FIFO_WR_ASSERT; 
    PORTH = FIFO_WR_DEASSERT;    

    PORTF = (data->tempRaw & 0xFF00)>>8;      //9th byte    
    PORTH = FIFO_WR_ASSERT; 
    PORTH = FIFO_WR_DEASSERT;
    
    PORTF = (data->tempRaw & 0xFF);           //10th byte    
    PORTH = FIFO_WR_ASSERT; 
    PORTH = FIFO_WR_DEASSERT;

    PORTF = (checksum & 0xFF00)>>8;           //11th byte    
    PORTH = FIFO_WR_ASSERT; 
    PORTH = FIFO_WR_DEASSERT;
    
    PORTF = (checksum & 0xFF);                //12th byte    
    PORTH = FIFO_WR_ASSERT; 
    PORTH = FIFO_WR_DEASSERT;  
}

void send_start_time(byte Hours, unsigned int Minutes, unsigned int Seconds, unsigned long start_usec_offset ){
