...
int init(){
    ...
    int request;
    if (!request = request_irq(IRQ, write_photon_data, SA_INTERRUPT, "read_fifo", NULL) )
    {// SA_INTERRUPT flag disables all other interrupts while this one is running
        printk(KERN_INFO "Couldn't register IRQ %i\n", IRQ);
        return -1;
    }
